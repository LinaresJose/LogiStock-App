...
class CostEntryModel {
  ...
  /// Columnas: A=costoId, B=skuId, C=fecha, D=costoUnitario, E=cantidad,
  ///           F=tipoOrigen, G=movimientoId, H=usuarioId, I=observacion
  factory CostEntryModel.fromRow(List<dynamic> row) {
    return CostEntryModel(
      costoId:      row.isNotEmpty ? row[0].toString() : '',
      skuId:        row.length > 1 ? row[1].toString() : '',
      ...
      usuarioId:    row.length > 7 ? row[7].toString() : '',
